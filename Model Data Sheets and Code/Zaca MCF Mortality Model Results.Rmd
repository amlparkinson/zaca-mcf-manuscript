...
rdnbr_mcf <- mcf %>% 
  ...
  count() %>% 
  rename(mcf_rdnbr = rdnbr_qualitative)

dnbr_mcf <- mcf %>% 
  group_by(dnbr_qualitative) %>% 
  count() %>% 
  rename(mcf_dnbr = dnbr_qualitative)

#coulter ------------------------------------------------------------------- 
rdnbr_pico <- pico %>% 
  group_by(rdnbr_qualitative) %>% 
  count() %>% 
  rename(pico_rdnbr =rdnbr_qualitative)

dnbr_pico <- pico %>% 
  group_by(dnbr_qualitative) %>% 
  count() %>% 
  rename(pico_dnbr = dnbr_qualitative)
...
rfm <- randomForest(mortality_qual ~ rain + tmax + TPI_five + dem_10m_zaca + flow_accumulation + sr + slope + TWI + cwd + vpd_aug + vpd_jan + aspect_transformed + ndvi, 
                    data = mcf, 
