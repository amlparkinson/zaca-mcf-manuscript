...
# all -------------------------------------------------------------------
all_sample_size <- cbind(mortality_count, rdnbr_mcf, dnbr_mcf, rdnbr_pico, dnbr_pico)
```
Yah, dnbr is not a good index to use. Rdnbr is better.
...
ggplot(mortality_count, aes(x = mortality_qual, y = n, fill = mort_type)) + geom_col(position = "dodge") +
...
## use lm to get a rdnbr threshold for the type converter stands? ---------------------
...
rfm <- randomForest(mortality_qual ~ rain + tmax + TPI_five + dem_10m_zaca + flow_accumulation + sr + slope + TWI + cwd + vpd_aug + vpd_jan + aspect_transformed, 
                    data = mcf, 
...
#sub data
mcf_ale <- mcf %>% 
  select(mortality_qual, rain, tmax, TPI_five, dem_10m_zaca, flow_accumulation, sr, slope, TWI, cwd, vpd_aug, vpd_jan) # the predictor$new code had issues with using a df with NA values even thought the variabels in the RF do not have NA values. 
...
rfm <- randomForest(two_class_mortality_mod ~ rain + tmax + TPI_five + dem_10m_zaca + flow_accumulation + sr + slope + TWI + cwd + vpd_aug + vpd_jan + aspect_transformed, 
...
mcf_ale <- mcf_2_class_mod %>% 
  select(two_class_mortality_mod, rain, tmax, TPI_five, dem_10m_zaca, flow_accumulation, sr, slope, TWI, cwd, vpd_aug, vpd_jan) # the predictor$new code had issues with using a df with NA values even thought the variabels in the RF do not have NA values. 
...
#vpd_jan
other <- plot(FeatureEffect$new(other_predictor, feature = "vpd_jan")) + ggtitle("Other")
high <- plot(FeatureEffect$new(high_predictor, feature = "vpd_jan")) + ggtitle("High")
...
rfm <- randomForest(tc_mortality ~ rain + tmax + TPI_five + dem_10m_zaca + flow_accumulation + sr + slope + TWI + cwd + vpd_aug + vpd_jan + aspect_transformed, 
                    data = mcf, 
...
```
variable importance is KINDA/MOSTLY stable. There arent big changes between runs, some variables flip 1 space, but some move 3 spaces. Variable importance was more stable when ndvi was included in the model.
...
rfm <- randomForest(rdnbr_qualitative2 ~ rain + tmax + TPI_five + dem_10m_zaca + flow_accumulation + sr + slope + TWI + cwd + vpd_aug + vpd_jan + aspect_transformed, 
                    data = mcf, 
