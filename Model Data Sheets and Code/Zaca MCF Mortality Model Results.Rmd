...
mcf <- read.csv('Zaca MCF variables for model.csv') %>% 
  select('drainage_association', 'ridge', 'mortality', 
         ...
         'TPI_queen', 'TWI', 'fri', 'dem_scaled', 'southness', "cwd", "vpd_jan", "vpd_aug", "tc_mortality" ) 
...
for (i in 1:50)
{
  ...
  #random forest model
  rfm<-randomForest(tc_mortality~ rain + tmax + TPI_five + dem_10m_zaca + flow_accumulation + sr + slope + TWI + cwd + vpd_aug + ridge + drainage_association, data = train, ntree=500, importance=T, nscale=0)
